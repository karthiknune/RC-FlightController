...
void mode_alt_hold_run(){
    ...
    if ((baro_data.healthy || gps_data.lock_acquired) && home_is_set()) {
        ...
        //Serial.printf("target alt:%.2f",target_agl);
        ...
        Serial.printf("despitch:%.2f\n", des_pitch);
        Serial.printf("actualpitch%.2f", actual_pitch);
    }

    g_des_pitch_setpoint = des_pitch;
