...
static void IRAM_ATTR isr_handler(void *arg)
{
    ...
    for (int i = 0; i < NUM_RX_CHANNELS; i++)
    {
        ...
        if (slot.unit == MCPWM_UNIT_0)
            MCPWM0.int_clr.val = bit_mask;
        else
            MCPWM1.int_clr.val = bit_mask;
    }
...
void rx_init()
{
    ...
    for (int i = 0; i < NUM_RX_CHANNELS; i++)
    {
        ...
        if (slot.unit == MCPWM_UNIT_0)
            int_mask_0 |= slot.int_ena_bit;
        else
            int_mask_1 |= slot.int_ena_bit;
    }
